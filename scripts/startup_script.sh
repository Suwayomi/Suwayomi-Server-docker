...
sed -i -r "s/server.webUIFlavor = ([a-zA-Z0-9\"]*)( #)?/server.webUIFlavor = \"${WEB_UI_FLAVOR:-\1}\" #/" /home/suwayomi/.local/share/Tachidesk/server.conf
sed -i -r "s/server.webUIChannel = ([a-zA-Z0-9\"]*)( #)?/server.webUIChannel = \"${WEB_UI_CHANNEL:-\1}\" #/" /home/suwayomi/.local/share/Tachidesk/server.conf
...
sed -i -r "s/server.authMode = ([a-zA-Z0-9\"]*)( #)?/server.authMode = \"${AUTH_MODE_VAL:-\1}\" #/" /home/suwayomi/.local/share/Tachidesk/server.conf
...
sed -i -r "s/server.opdsChapterSortOrder = ([a-zA-Z0-9\"]*)( #)?/server.opdsChapterSortOrder = \"${OPDS_CHAPTER_SORT_ORDER:-\1}\" #/" /home/suwayomi/.local/share/Tachidesk/server.conf
...
sed -i -r "s/server.koreaderSyncChecksumMethod = ([a-zA-Z0-9\"]*)( #)?/server.koreaderSyncChecksumMethod = \"${KOREADER_SYNC_CHECKSUM_METHOD:-\1}\" #/" /home/suwayomi/.local/share/Tachidesk/server.conf
sed -i -r "s/server.koreaderSyncStrategy = ([a-zA-Z0-9\"]*)( #)?/server.koreaderSyncStrategy = \"${KOREADER_SYNC_STRATEGY:-\1}\" #/" /home/suwayomi/.local/share/Tachidesk/server.conf
...
# database
sed -i -r "s/server.databaseType = ([a-zA-Z0-9\"]*)( #)?/server.databaseType = \"${DATABASE_TYPE:-\1}\" #/" /home/suwayomi/.local/share/Tachidesk/server.conf
